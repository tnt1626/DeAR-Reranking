...
def main():
    print("=== MEDPRS JOURNAL RECOMMENDATION INFERENCE PIPELINE (QWEN 16-BIT HIGH-SPEED) ===")
    
    import argparse
    parser = argparse.ArgumentParser(description="MedPRS journal recommendation inference with high-speed Qwen 16-bit.")
    ...
    # 4. Load Qwen 2.5 7B Instruct Model in 16-bit (VRAM heavy but 2x-3x faster)
    print("Loading Qwen-2.5-7B-Instruct Model in 16-bit precision...")
    qwen_repo = "Qwen/Qwen2.5-7B-Instruct"
    qwen_tokenizer = AutoTokenizer.from_pretrained(qwen_repo, use_fast=True)
    
    dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    
    # Load Qwen base model split across both GPUs via device_map="auto"
    ...
    for i in range(num_eval_papers):
        ...
        # 6. Construct Listwise Prompt for Qwen (Optimized for maximum accuracy and reasoning quality)
        SYSTEM_PROMPT = (
            ...
            "Explain your reasoning in detail first, and then write the final ranking at the very end of your response."
        )
        ...
        post_prompt = (
            ...
            "Please follow these steps to perform a deep analysis:\n"
            "Step 1: Write a detailed 2-to-3 sentence analysis for each journal. Evaluate the match between the paper's specific research focus/methodology and the journal's scope, identifying clear points of alignment or mismatches.\n"
    ...
    # Save copy out to /kaggle/working/results to survive Git exclusions
